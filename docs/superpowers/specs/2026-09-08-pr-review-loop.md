...
| C55 | The review is posted by the TRIGGER, never by the reviewer. The reviewer generates its review once and `Bash(gh pr:*)` stays denied to it, so it cannot comment, merge or close. Posting happens last — after the ledger row and the findings file are both on disk — so a GitHub failure costs the convenience of reading the review there and nothing else, and the wake says so rather than letting A assume the comment exists. | confirmed — 4 negative controls |

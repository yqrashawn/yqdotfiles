...
| C58 | That mismatch has two causes needing opposite handling: GitHub has not caught up (wait) or this push was superseded (drop). Only time separates them, so the PR head is re-asked a bounded number of times and then dropped — a superseded push pays the sleep in a background hook, and the newer push has its own trigger. Unbounded retry hangs the hook: a negative control without the bound had to be killed. | confirmed — 3 negative controls |

...
you to write `VERDICT[<tag>]:`. It is REQUIRED, and it is the only thing that
separates your answer from a previous pass's verdict you quote while verifying
closure, which you are asked to do. **A reply with no tagged verdict at column 0
is discarded** — the whole review is thrown away and re-run, however good the
findings were. If more than one tagged verdict appears, the last is taken, so do
not restate the format with the tag before reviewing.

    VERDICT[<tag>]: MERGEABLE — N follow-ups to file
    VERDICT[<tag>]: NOT MERGEABLE — <shortest statement of the blocking finding>

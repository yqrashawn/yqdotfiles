  ...
  (let [spawn (or (:spawn-fn opts) default-spawn)
    ...
    (try
      ...
      (let [res (binding [*token* token]
        ...
        ;; working token — until whatever reset the quoted text names.
        ;; `tokens/reset-at-ms` reads a reset only from the LINE a banner
        ;; starts on, so quoted prose cannot supply one; a quoted whole
        ;; banner LINE still can, and this exit gate is what stops it.

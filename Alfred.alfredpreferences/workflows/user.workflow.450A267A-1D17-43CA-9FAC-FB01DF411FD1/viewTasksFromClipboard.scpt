on run argv
	...
	set tps to lib's getTaskParameters(q, false, "clipboard_folder") -- {taskTags, taskDesc, taskGroup, taskGroupType, taskNote, taskDate}
	...
	set xml to xml & "Create following tasks from clipboard content:"
	set xml to xml & "</title>" & linefeed
	set xml to xml & "<subtitle>" 

	-- group:	
	if taskGroup equal to "" then 
		set taskGroup to lib's getKeyValue("clipboard_folder") -- default folder assignment
	end if
	...
	if taskGroupType = "" then set xml to xml & taskGroup
	if taskGroupType = "a" or taskGroupType = "p" or taskGroupType = "s" then set xml to xml & q_encode(taskGroup)

	-- tags:	
	set xml to xml & " Tags (@): "
	if taskTags is not "" then 
		set xml to xml & q_encode(taskTags)
	else
		set xml to xml & "-"
	end if
	...
	if taskNote is not "" then 
		set xml to xml & q_encode(taskNote)
	else
		set xml to xml & "-"
	end if
	...
	repeat with myLine in myText
		if length of myLine is greater than 0 then
			...
			set xml to xml & linefeed & "<item uid=\"new_task" & c & "\" arg=\"" & q & "\" valid=\"yes\">" & linefeed
			set xml to xml & "<title>"
			set xml to xml & q_encode(myLine)
			set xml to xml & "</title>" & linefeed
			set xml to xml & "<subtitle></subtitle>" & linefeed
			set xml to xml & "<icon>icons/task.png</icon>" & linefeed
			set xml to xml & "</item>" & linefeed

  ...
  launchd.daemons.tailscaled = {
    serviceConfig = {
      ...
      ProgramArguments = [
        "/bin/sh"
        "-c"
        "${pkgs.tailscale}/bin/tailscaled -tun userspace-networking -statedir /Users/${config.user.name}/tailscale -socks5-server=localhost:6156 -outbound-http-proxy-listen=localhost:6156"

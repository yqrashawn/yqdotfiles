(custom-set-variables
 ...
 '(safe-local-variable-values
    '((cider-shadow-watched-builds . ":app") (cider-jack-in-default quote babashka)
       ...
       (major-mode . emacs-lisp-mode) (apheleia-formatter . zprint)
       ...
       (eval add-hook 'before-save-hook
         ...
         nil t)
       (org-edit-src-content-indentation 0) (vc-prepare-patches-separately)
       (diff-add-log-use-relative-names . t) (vc-git-annotate-switches . "-w")
       (+cljr--log-spy-with-error . t) (+clojure-use-zprint-formatter . t)
       ...
       (eval progn (pp-buffer) (indent-buffer)) (js2-mode-show-strict-warnings)
       (js2-mode-show-parse-errors)
       (eval define-clojure-indent (l/matcha '(1 (:defn)))
         (l/matche '(1 (:defn))) (p.types/def-abstract-type '(1 (:defn)))
       ...
       (counsel-rg-base-command "rg" "--max-columns" "240" "--with-filename"
         "--no-heading" "--line-number" "--color" "never" "-uu" "%s")
       ...
       (checkdoc-package-keywords-flag) (buffer-save-without-query . t)
       ...
       (eval defun +jest-integration-test-file-p nil
         (and (buffer-file-name)
           (string-match-p "\\.integration.test\\.js" (buffer-file-name))))
       ...
       (eval defun +insert-jest-expect nil
         (save-excursion
           ...
           (when (not (re-search-forward "from.*@jest/globals" (point-max) t))
             (insert
               "// eslint-disable-next-line no-unused-vars\12import {expect, describe, test, it, jest, afterAll, afterEach, beforeAll, beforeEach} from '@jest/globals' // prettier-ignore\12")
             (insert
               "// eslint-disable-next-line no-unused-vars\12import waitForExpect from 'wait-for-expect'\12"))))

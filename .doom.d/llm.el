...
(defun +gptel (arg)
  (interactive "P")
  (cond
   ((and (region-active-p) (eq arg 8)) (call-interactively #'+gptel-comment-region))
...
(use-package! gptel
  ...
  ;; Cache llm-danger-buffer-p results per buffer using content hash.
  ;; gitleaks spawns a subprocess (~130ms per call) — skip when content unchanged.
  (defvar-local +llm-danger--cache-hash nil)
  (defvar-local +llm-danger--cache-result nil)

  (defun +llm-danger-buffer-p-cached (buf)
    "Like `llm-danger-buffer-p' but caches result by content hash.
Avoids repeated gitleaks subprocess spawns for unchanged buffers."
    (comment
      (with-current-buffer buf
  ...
  (defun +gptel-context-add-buffer (b)
    (unless (+llm-danger-buffer-p-cached b)
      (with-current-buffer b
        (gptel-context--add-region b (point-min) (point-max) t))))

  (defun +gptel-context-add-buffer-unchecked (b)
    "Add buffer B to gptel context without danger check.
For buffers generated by our own code that cannot contain secrets."
  ...
  (defadvice! +before-gptel-make-fsm (&optional _args)
    ...
    (unless simple-llm-req-p
      ...
      ;; Build context buffers once (each call erases+rebuilds content)
      (let ((ws-info-buf (+current-workspace-info-buffer))
            (vis-buf (+visible-buffers-list-buffer))
            (lint-buf (+current-workspace-lints-buffer)))
        ;; Remove managed context buffers
        (dolist (buf (list ws-info-buf vis-buf lint-buf))
          (setf (alist-get buf gptel-context nil 'remove) nil))
        ;; Re-add generated buffers (skip gitleaks — we control their content)
        (+gptel-context-add-buffer-unchecked ws-info-buf)
        (+gptel-context-add-buffer-unchecked vis-buf))
      ;; (+gptel-context-add-buffer (+magit-wip-diff-n-min-buffer 5))
      ;; Add project default files as context

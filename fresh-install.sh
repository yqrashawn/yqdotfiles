...
brew cask install alacritty iina notion podcastmenu bitbar key-codes qlmarkdown visual-studio-code quitter kitematic spectacle squirrel flux mpv telegram the-unarchiver setapp kawa anybar
brew install ack ffmpeg openssl sqlite html2text optipng sshrc findutils htop flac httpie hub p7zip fontconfig pandoc aria2 imagemagick pcre autoconf libyaml pcre2 terminal-notifier autoconf-archive fx imapfilter libzip perl autojump fzf pinentry texi2html automake lua pinentry-mac texinfo bash the_platinum_searcher bash-completion gcc luajit pkg-config the_silver_searcher ispell lumo planck thefuck bfg isync m-cli pngquant tmate macvim poppler tmux jid mailcheck topgrade mailutils jpeg proxychains-ng trash brew-cask-completion maven proxytunnel chicken git languagetool mongodb chrome-cli git-extras leiningen pyenv chunkwm git-flow mpfr unixodbc clang-format git-lfs less mpv cliclick git-standup python clipper python3 clojure multimarkdown clojurescript mycli wabt cmake n coreutils qt cscope ctags neovim ranger wget curl readline reattach-to-user-namespace redis diff-so-fancy gnu-sed nginx ripgrep gnupg gnupg2 nnn gnutls libogg node yarn doxygen go ruby editorconfig rust you-get goku youtube-dl exa zsh fasd grep fcitx-remote-for-osx fd
